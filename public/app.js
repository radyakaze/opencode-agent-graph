;(() => {
  ...
  function updateElapsedTimers() {
    ...
    els.list.querySelectorAll('[data-elapsed]').forEach((timer) => {
      ...
      const elapsed =
        Number.isFinite(base) && Number.isFinite(renderedAt)
  ...
  const projectId = (value, index) =>
    `live-project-${index}-${String(value)
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')}`
  ...
  function mergeProjectAgents(project) {
    ...
    registered.forEach((registeredAgent) => {
      const matchIndex = active.findIndex(
        (agent, index) => !usedActive.has(index) && agent && agent.name === registeredAgent.name,
      )
      if (matchIndex >= 0) {
  ...
  function topologyMarkup(project, index) {
    ...
    const activeCount = agents.filter(
      (agent) => agent.status === 'busy' || agent.status === 'retry',
    ).length
    ...
    const parts = agents.map((agent, agentIndex) => {
      ...
      const live =
        status === 'retry' || (status === 'busy' && hasRecentActivity(agent.lastActivityAt))
      ...
      const packetMarkup = active
        ? Array.from({ length: status === 'retry' ? 1 : 3 }, (_, packetIndex) => [
            `<circle class="cable-packet live-packet packet-${packetIndex} ${status === 'retry' ? 'retry' : ''}" data-path="${pathId}" data-offset="${offset + packetIndex * 0.19}" data-direction="forward" r="${packetIndex === 0 ? 4.5 : 3.5}" aria-hidden="true"></circle>`,
            `<circle class="cable-packet live-packet packet-${packetIndex} ${status === 'retry' ? 'retry' : ''}" data-path="${pathId}" data-offset="${offset + packetIndex * 0.19 + 0.09}" data-direction="reverse" r="${packetIndex === 0 ? 4 : 3}" aria-hidden="true"></circle>`,
          ])
            .flat()
            .join('')
        : ''
      ...
      const sessionSuffix =
        active && agent.sessionId ? ` · ${String(agent.sessionId).slice(-6)}` : ''
    ...
    packetNodes.push(
      ...paths
        ...
        .map((path) => ({ ...path, selector: `${uid}-cable-${paths.indexOf(path)}` })),
    )
  ...
  function updateLiveness() {
    ...
    packetNodes.forEach((packet) => {
      const live =
        packet.retry || (packet.status === 'busy' && hasRecentActivity(packet.lastActivityAt, now))
      const path = document.getElementById(packet.pathId)
      if (path) path.classList.toggle('no-activity', packet.status === 'busy' && !live)
      document
        .querySelectorAll(`.live-agent-node[data-path="${packet.pathId}"]`)
        .forEach((node) => {
  ...
  function animatePackets(time) {
    packetNodes.forEach((packet) => {
      ...
      elements.forEach((element) => {
        const path = document.getElementById(packet.pathId)
        if (!path) return
        const raw = (time * (packet.retry ? 0.00016 : 0.00042) + Number(element.dataset.offset)) % 2
        const pingPong = raw <= 1 ? raw : 2 - raw
        const burst = packet.retry
          ? (time * 0.0011 + Number(element.dataset.offset) * 3.7) % 4.4 < 0.9
          : true
  ...
  function positionTooltip(node) {
    ...
    const caretOffset = Math.max(
      -maxCaretOffset,
      Math.min(maxCaretOffset, nodeCenterX - tipCenterX),
    )
